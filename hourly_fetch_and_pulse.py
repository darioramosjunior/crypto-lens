...
def calculate_price_changes(in_memory_data):
    ...
    try:
        for symbol, df in in_memory_data.items():
            ...
            df_sorted['previous_close'] = df_sorted['close'].shift(1)
            df_sorted['price_change_1h'] = ((df_sorted['close'] - df_sorted['previous_close']) / df_sorted['previous_close'] * 100)
            
            # Keep only the latest row with valid price change
            valid_rows = df_sorted[df_sorted['previous_close'].notna()][['timestamp', 'close', 'previous_close', 'price_change_1h']].copy()
            if len(valid_rows) > 0:
                latest_row = valid_rows.iloc[-1:].copy()
                latest_row['symbol'] = symbol
                price_changes.append(latest_row)
...
if __name__ == "__main__":
    ...
    # Step 2: Fetch OHLCV data asynchronously and store in memory
    ...
    # Step 3: Parse raw data into DataFrames and keep in memory
    ...
    if generate_rsi_sentiment_chart(indicators_data):
        ...
        try:
            ...
            print("✓ RSI sentiment chart uploaded to Discord")
        except Exception as e:
