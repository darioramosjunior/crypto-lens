...
log_path = os.path.join(script_dir, "logs", "hourly_dc_log.txt")
    ...
    try:
        async with session.get(BASE_URL, params=params, timeout=10) as response:
            if response.status == 200:
                ...
                logger.log_event(log_category="ERROR", message=f"Failed to fetch OHLCV for symbol {symbol} with status {response.status}")
                return symbol, None
    except Exception as e:
...
if __name__ == "__main__":
    ...
    for symbol, data in results.items():
        ...
        calculate_and_save_indicators(symbol)